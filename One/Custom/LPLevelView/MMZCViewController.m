...
@property (nonatomic,retain)UITextField *loguptf;
@end
...
//    self.navigationItem.rightBarButtonItem=right;
    ...
    UIButton *login =[[UIButton alloc]initWithFrame:CGRectMake((self.view.frame.size.width-30)/2, 30, 50, 30)];
    [login setTitle:@"登录" forState:UIControlStateNormal];
    [login setTitleColor:[UIColor colorWithRed:248/255.0f green:144/255.0f blue:34/255.0f alpha:1] forState:UIControlStateNormal];
    [login addTarget:self action:@selector(login:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:login];

    
    UIButton *smsLogup =[[UIButton alloc]initWithFrame:CGRectMake(15, 200, (self.view.frame.size.width-30)/1, 230)];
    [smsLogup setTitle:@"短信注册" forState:UIControlStateNormal];
    [smsLogup setTitleColor:[UIColor colorWithRed:248/255.0f green:144/255.0f blue:34/255.0f alpha:1] forState:UIControlStateNormal];
    [smsLogup addTarget:self action:@selector(logup:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:smsLogup];
...
- (void)logup:(UIButton *)btn{
    
    YJViewController *useragreementvc = [[YJViewController alloc] init];
    
    useragreementvc.str = self.loguptf.text;
    //：使用导航控制器推出下一个试图控制器
